...
@SpringBootTest
public class VerifyNegativeValAfterDiscountTests {

//to verify negative cart values in the case of flat discoount offer

	@Test
	public void applyFlatXOfferNegativeTest() throws Exception {

		Boolean isNegative = false;
	    resetOffers();
	    List<String> segments = new ArrayList<>();
	    segments.add("p1");
	    OfferRequest offerRequest = new OfferRequest(1, "FLATX", 420, segments);
	    addOffer(offerRequest);

	    // Step 2: Apply offer
	    ApplyOfferRequest applyOfferRequest = new ApplyOfferRequest(1, 1, 200); 
	    ApplyOfferResponse response = applyOffer(applyOfferRequest);
	    System.out.println(response.getCart_value()+"sax768");
	    if(response.getCart_value()<0){
	    	isNegative = true;
	    }
	    System.out.println("hey"+isNegative);
	    Assert.assertEquals(isNegative, true);
	}
